...
import { Admin, AppContent } from 'collections/schema'
import {
  getDocumentById,
...
class ApplicationContent {
  updateAppContent = async (
    values: UpdateAppContentValues
  ): Promise<AppContent | DocumentData> => {
    return new Promise<AppContent | DocumentData>(async (resolve, reject) => {
      try {
        ...
        const updatedSnapshot = await getDoc(
          doc(firestore, COLLECTIONS.APP_CONTENTS, docId)
        )
  ...
  getAppContents = (): Promise<AppContent[]> => {
    return new Promise(async (resolve, reject) => {
      try {
        const appContentsCollectionRef = collection(
          firestore,
          COLLECTIONS.APP_CONTENTS
        )
