...
import { Admin, User } from 'collections/schema'
  ...
  getUsersData = (): Promise<any> => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Wavelength
        const findFocusUsers = finfFocusUsersSnapshot.docs.map((doc) =>
          doc.data()
        )
        const moveMotivatedUsers = moveMotivatedUsersSnapshot.docs.map((doc) =>
          doc.data()
        )
        const stressLessUsers = stressLessUsersSnapshot.docs.map((doc) =>
          doc.data()
        )

        // Pace
        const easyWalkUsers = easyWalkUsersSnapshot.docs.map((doc) =>
          doc.data()
        )
        const eightMinutePerMileUsers =
          eightMinutePerMileUsersSnapshot.docs.map((doc) => doc.data())
        const moderateWalkUsers = moderateWalkUsersSnapshot.docs.map((doc) =>
          doc.data()
        )
        const tenMinutePerMileUsers = tenMinutePerMileUsersSnapshot.docs.map(
          (doc) => doc.data()
        )
        const twelveMinutePerMileUsers =
          twelveMinutePerMileUsersSnapshot.docs.map((doc) => doc.data())

        // Commitment
        const everydayUsers = everydayUsersSnapshot.docs.map((doc) =>
          doc.data()
        )
        const oneToTwoPerWeekUsers = oneToTwoPerWeekUsersSnapshot.docs.map(
          (doc) => doc.data()
        )
        const twoToFourPerWeekUsers = twoToFourPerWeekUsersSnapshot.docs.map(
          (doc) => doc.data()
        )
